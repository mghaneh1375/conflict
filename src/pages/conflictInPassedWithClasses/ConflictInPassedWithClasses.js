...
function ConflictInPassedWithClasses() {
  ...
  useEffect(() => {
    ...
    Promise.all([_GET(routes.conflictInPassedWithClasses, state.token)]).then(
      (res) => {
  ...
  const removeFromPassed = async (classId, userId) => {
    dispatch({ loading: true });
    const res = await _POST(
      routes.removeClassFromPassed + classId + "/" + userId,
      undefined,
      state.token
    );
    dispatch({ loading: false });
  ...
  const addUserToClass = async (classId, userId) => {
    dispatch({ loading: true });
    const res = await _POST(
      routes.addUserToClass + classId + "/" + userId,
      undefined,
      state.token
    );
    dispatch({ loading: false });
...
export default ConflictInPassedWithClasses;
